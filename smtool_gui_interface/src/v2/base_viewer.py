...
class BaseViewer(QLabel):
    ...
    def paintEvent(self, event):
        if self.image is not None:
            ...
            if img_to_draw is None:
                ...
                img_to_draw = self.get_visual_image3()
            pixmap = QPixmap(utils.cvImage2QImage(img_to_draw))
    ...
    def wheelEvent(self, event):
        ...
        if self.image_orig is not None:
            if -self.pos_00[0] + event.pos().y() > self.zoom_hw[0] or -self.pos_00[1] + event.pos().x() > self.zoom_hw[1]:
                return
    ...
    def mousePressEvent(self, event):
        if event.buttons() == Qt.RightButton:
            self.move_shift[0] = event.pos().y()
            self.move_shift[1] = event.pos().x()

    def mouseReleaseEvent(self, event):
        if self.move_shift != [0, 0]:
            self.move_shift[0] = event.pos().y() - self.move_shift[0]
            self.move_shift[1] = event.pos().x() - self.move_shift[1]

            self.pos_00[0] += self.move_shift[0]
            self.pos_00[1] += self.move_shift[1]
            self.normalize_pos00()

            self.move_shift = [0, 0]

            self.move_drag.emit(self.zoom_hw, self.pos_00)
            self.update()